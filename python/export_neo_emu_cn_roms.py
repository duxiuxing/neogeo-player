...
class ExportNeoEmuCnRoms(CmdHandler):
    def __init__(self):
        super().__init__("NEO.emu.cn - 导出 - 中文命名的 ROM")
    ...
    def run(self):
        ...
        xml_path = os.path.join(MainMenu.console.root_folder_path(),
                                "NEO.emu.cn\\roms_export.xml")
        if not os.path.exists(xml_path):
            print(f"无效的文件：{xml_path}")
            return
        tree = ET.parse(xml_path)

        root_folder_path = os.path.join(LocalConfigs.sd_path(), "roms\\NEO")
